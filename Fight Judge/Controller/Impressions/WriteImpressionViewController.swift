...
class WriteImpressionViewController: UIViewController,UINavigationControllerDelegate,UITextViewDelegate {
    ...
    @IBOutlet var impressionTextView: UITextView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //データの読み込み
        let query = NCMBQuery(className: "FightImpression")
        //ユーザーネーム生成
        let userName = NCMBUser.current()?.object(forKey: "userName") as? String
        
        //感想であることを絞り込み
        query?.whereKey("type", equalTo: "impression")
        //同じ日時の投稿を絞り込み
        ...
        query?.findObjectsInBackground({ (result, error) in
            if error != nil {
                ...
                let impression = text?.last?.object(forKey: "impression") as? String
                if title == nil || impression == nil {
                    self.saveButton.isEnabled = false
                    self.postButton.isEnabled = false
                    self.titleTextVIew.delegate = self
                    self.impressionTextView.delegate = self
                    
                } else {
                    self.titleTextVIew.text = title
                    self.impressionTextView.text = impression
                }
                ...
                if  result as! [NCMBObject] != [] {
                    ...
                    query?.findObjectsInBackground({ (result, error) in
                        if error != nil {
                            ...
                            textObject?.setObject(self.impressionTextView.text, forKey: "impression")
                    ...
                    let object = NCMBObject(className:"FightImpression")
                    //ユーザー情報を追加
                    ...
                    object?.setObject(self.impressionTextView.text, forKey: "impression")
                    //試合日時を識別子として追加する
    ...
    func confirmContent() {
        if titleTextVIew.text.count > 0 && impressionTextView.text.count > 0 {
    ...
    @IBAction func upload() {
        ...
        uploadedObject?.setObject(impressionTextView.text, forKey: "text")
        uploadedObject?.setObject("impression", forKey: "type")
        //試合日時を識別子として追加する
